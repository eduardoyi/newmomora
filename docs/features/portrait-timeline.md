...
| `illustrated_profile_key` | Last successful generated portrait key. Always `.webp`-suffixed, but the REAL bytes underneath are not guaranteed to be webp: OpenAI's images/edits endpoint has been observed to ignore the requested `output_format: 'webp'` and return PNG bytes anyway. `generate-portrait-illustration` sniffs the real bytes and re-encodes a mismatch to a smaller JPEG before upload (`_shared/image-bytes.ts`'s `resolveRealImageBytesForStorage`), but keeps the `.webp` extension — `claim_family_member_portrait_generation` (20260722130000_portrait_generation_workflow_jobs.sql) validates that exact extension server-side BEFORE generation even runs, so the key can't legitimately vary by real format without a dedicated migration relaxing that check (not done as part of this fix; tracked separately). Any consumer reading this key must sniff the real bytes rather than trust the extension — `compose-share-card`'s `resolveImageMimeType` already does. Existing mismatched rows are cleaned up by `supabase/scripts/backfill-portrait-reencode.ts`, which — unlike the live generator — CAN mint a fresh, extension-correct key (it isn't bound by the claim RPC's format check) and does so for this column; see [memory-sharing.md's changelog](./memory-sharing.md#changelog) for the incident history. |
...
- Both the frozen source photo and style reference are mandatory; Cloudflare resizes each to 1024px, sends style first and source second, writes WebP, and never falls back to text-only generation. `gpt-image-2.5-flare` gets one bounded attempt; retryable provider failures may use one reference-aware `gpt-image-1.5` attempt with high input fidelity.
