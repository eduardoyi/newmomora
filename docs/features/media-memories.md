...
- Tapping a Timeline carousel opens the detail screen **on the asset the parent was looking at**, not the first one: the card forwards the tapped page index to `memoryDetailRoute(memoryId, mediaIndex)`, and the detail carousel opens there (`initialIndex`). Tapping the caption or footer below the carousel has no page context and opens on the first asset, as does any other entry point (calendar, family profile, notification deep link).
...
- **Full-screen zoom (photos and illustrations):** in the full-screen viewer, a pinch zooms the image up to 4x around the point between the fingers, and a double tap toggles between fitted and 2x anchored on the tapped point. While zoomed, one finger pans the image, clamped to the visible image edges (never into the letterbox), and swipe paging is suspended until the image is pinched or double-tapped back to fit. Videos are not zoomable — a tap still toggles play/pause. Zoom never persists: moving to another item restores that page fitted.
...
- **Media replacement on edit:** Mint a fresh `mediaAssetId`/object key for the replacement (same pattern `uploadMemoryMediaAssets` already uses for a newly-attached asset) and update the row's `object_key`/`preview_object_key` to point at it; then delete the old object. Do **not** PUT the replacement over the existing key — see the cacheKey constraint below.
...
- **Retained-preview admission is tied to the original key, not merely family membership** — `replace_memory_media_assets` snapshots `object_key → preview_object_key` before its delete/reinsert. An owner/manager may retain an existing preview under another member's immutable uploader prefix only when it is the exact preview paired with the same retained original. A new/replaced preview remains caller-prefix constrained, and a forged foreign preview or a preview swapped from a different existing asset is rejected. Keep this pairing check when changing the RPC; accepting any preview key found elsewhere in the memory would create a media-reference substitution path.
...
- **Never overwrite media in place under a reused key.** `object_key`/`preview_object_key`/`illustration_key`/`illustrated_profile_key` are all treated as immutable once written — a new upload/regeneration always mints a fresh key rather than PUTting over an existing one (`uploadMemoryMediaAssets` only re-uploads assets with no existing `objectKey`; illustration regeneration and portrait versions are the same pattern). Every R2-backed `expo-image` site pins `cacheKey` to the object key (`src/utils/media-image-source.ts`, Workstream O5 of `docs/plans/offline-awareness-and-share-cards.md`) precisely because that invariant holds — a device with the old bytes cached under that key would keep serving them forever if a future change ever overwrote a key in place. Any extension that touches media/illustration/portrait storage must preserve "one key, one set of bytes, forever."
...
| `supabase/tests/retained_media_preview_authorization.sql` | Owner/manager cross-creator retained-preview admission, manager retain/add/reorder with omitted retained previews, caller-owned additions, forged/swapped foreign-preview rejection with rollback, viewer/non-member denial, and metadata preservation |
