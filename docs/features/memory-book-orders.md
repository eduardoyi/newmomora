...
- A **"Ships to" block** on `OrderStatusScreen` (owner request
  2026-09-09): the row's `shipping_address` (persisted by the `quote` op,
  buyer-readable under the same RLS as the rest of the row) rendered
  under the price facts once it exists (`quoted` onward), with a
  "Wrong address? Reply to your confirmation email right away" hint shown
  only for `paid`/`rendering`/`submitted`/`in_production` — the statuses
  where a fast reply can still catch a mistake, backed by the KEPT 2h
  Prodigi edit window (owner decision 2026-09-09,
  `docs/plans/prodigi-order-spec.md`). Pre-payment (`quoted`) the buyer
  can simply start over; post-`shipped` it's too late for a fix but the
  address stays visible for the support conversation.
...
## Implementation (address-entry autofill round, this change)

Two-tier improvement to the checkout address step (`book-renderer/src/web/order/`), on top of the shipped wave-3 checkout UI and the order-status UX round. Server contract UNCHANGED — `ShippingAddressInput` and the `quote` op are exactly as before; both tiers just get a parent to a valid `ShippingAddressInput` faster.

- **Tier 1 — autofill markup hygiene on the manual form** (`order/AddressForm.tsx`, extended, not replaced): every input now carries a `name` attribute AND an `autoComplete` token prefixed with the `shipping` section keyword (WHATWG "autofill detail tokens" — `shipping name`, `shipping address-line1`, `shipping address-line2`, `shipping address-level2`, `shipping address-level1`, `shipping postal-code`, `shipping country`) in place of the bare tokens it used before. Scopes a browser/password-manager autofill suggestion to a *shipping* address specifically. Nothing else about the manual form changed — it's still the keyboard-safe, `SHIPS_TO_COUNTRIES`-limited form documented above, and it's now also the permanent fallback for Tier 2 (below).
- **Tier 2 — Stripe Address Element as the primary path** (new: `order/AddressStep.tsx`, `order/StripeAddressForm.tsx`, `order/stripeClient.ts`, `order/stripeAddressMapping.ts`): `CheckoutScreen.tsx` now renders `AddressStep` instead of `AddressForm` directly. `AddressStep` picks between the two paths:
  - **Stripe's Address Element** (type-ahead address autocomplete, `@stripe/react-stripe-js` + `@stripe/stripe-js`, free via the existing Stripe account) when NOT in fixture mode, `VITE_STRIPE_PUBLISHABLE_KEY` is non-empty, and `loadStripe()` resolves to a real `Stripe` instance.
  - The **manual `AddressForm`** (Tier 1, above) for every other case — missing/empty key, fixture mode, or ANY `loadStripe()` failure (blocked script, bad key, network hiccup). Falls back silently: a parent never sees a broken checkout step because a third-party script didn't load.
  - `stripeClient.ts#resolveStripe()` is the only caller of `loadStripe` — lazy (only invoked when the address step actually renders, never at module eval/app boot) and memoized once per session (a module-level `Promise` reused on every call, so re-opening checkout doesn't reload Stripe's script again). Fixture mode is checked FIRST and synchronously (`import.meta.env.DEV && getFixtureSlug()`, same gating every other fixture-aware call site in this app uses) so a fixture session never even attempts the network fetch of Stripe's loader script.
  - `StripeAddressForm.tsx` wraps `AddressElement` (mode `'shipping'`, `display: { name: 'full' }` so it collects the recipient name, `allowedCountries` built from the SAME `SHIPS_TO_COUNTRIES` list the manual form's `<select>` uses — this path can't quote a destination the manual path couldn't either, `autocomplete: { mode: 'automatic' }` — Stripe's default. **Autocomplete correction (2026-09-09, owner-verified live):** Stripe's free Google-Maps type-ahead only applies when the Address Element is paired with the Payment Element; used standalone — our case, payment is hosted Checkout — `automatic` mode provides NO suggestions, and enabling them requires the owner's own Google Maps Places API key via `autocomplete: { mode: 'google_maps_api', apiKey }` (paid Google Cloud product; Stripe's supported-country list also excludes PT regardless). Until then the Element's value is per-country field localization + validation, with browser/password-manager autofill covering the type-ahead role) in an `<Elements>` provider with no `clientSecret` (fine — the Address Element doesn't need a PaymentIntent/SetupIntent context) and a minimal `appearance` (colorPrimary/fontFamily/borderRadius copied from `theme.ts`'s `colors.primary`/`radius.md`, `fonts.cssSrc` pointing at the SAME Google Fonts `<link>` `web.html` already loads — nothing new added to the page's font budget). Listens to the Element's `change` event; the existing "Get quote" submit button stays disabled until `event.complete` is true, then submits `toShippingAddressInput(event.value)` through the exact same `onSubmit` prop the manual form uses.
  - `stripeAddressMapping.ts#toShippingAddressInput` is a pure mapping function (deliberately does NOT import `@stripe/stripe-js` types — a minimal local `StripeAddressValue` interface instead) so it stays unit-testable under this package's jsdom-less vitest config; trims fields, maps an empty/whitespace line2/city/state to `undefined`, passes `country` through unchanged as `countryCode`. Unit-tested in `order/__tests__/stripeAddressMapping.test.ts`.
  - Keyboard-safe (repo UX rule, unchanged): the Address Element renders inside an iframe Stripe controls, but the submit button below it stays in normal document flow — never `position: fixed` — same as the manual form.
- **New env var — `VITE_STRIPE_PUBLISHABLE_KEY`** (build-time, public config — a Stripe publishable key is safe to expose client-side by design, same posture as `VITE_SUPABASE_ANON_KEY`; typed as optional in `vite-env.d.ts`). **Deploy prerequisite, not yet done by this change:** the owner needs to add this to `book-renderer/.env.local` (for `npm run dev:web`) AND to whatever env source the production `build:web` uses (e.g. the CI/deploy job's env, however `VITE_SUPABASE_URL`/`VITE_SUPABASE_ANON_KEY` already reach that build) — until then, `shop.usemomora.com` runs Tier 1 (manual form) only, silently, exactly as designed.
- **Bundle gate**: `npm run build:web` (`tsc --noEmit && vite build ... && check:web-bundle`) stays green — verified. `@stripe/stripe-js` ships a small loader in the built bundle (it fetches the real `stripe.js` from `js.stripe.com` at runtime, not bundled) — production `web-*.js` grew ~530KB → ~545KB minified. `check-web-bundle.mjs`'s fixture-string scan still passes with zero occurrences (same `import.meta.env.DEV`-gated `getFixtureSlug()` call pattern every other fixture-aware module already uses, dead-code-eliminated the same way).
- **Tests**: `order/__tests__/stripeAddressMapping.test.ts` (new — full mapping, trimming, null/empty→undefined for line2/city/state, country passthrough). `npx tsc --noEmit` clean; `npx vitest run` 511/511 (up from 504 baseline, +6 new + 1 net for renamed suite adjacency — see the actual file list above for the exact new suite).
- **Interactive verification**: fixture mode (`?fixture=<slug>` → "Order this book") confirmed to still render the MANUAL form with no console errors (fixture mode never attempts `loadStripe`). The live Address Element path needs the owner's `VITE_STRIPE_PUBLISHABLE_KEY` to verify end to end — not yet done locally; the fallback-selection logic (no key → manual form, no crash) was verified instead.
...
| 2026-09-09 | Checkout address entry improved, two tiers: Tier 1 — `order/AddressForm.tsx`'s inputs now carry `name` + `shipping`-prefixed `autoComplete` tokens (WHATWG autofill detail tokens) so browser/password-manager autofill targets a shipping address specifically. Tier 2 — Stripe's Address Element (`@stripe/react-stripe-js` + `@stripe/stripe-js`, new deps) as the primary address input when available (new: `order/AddressStep.tsx`, `order/StripeAddressForm.tsx`, `order/stripeClient.ts`, `order/stripeAddressMapping.ts`), falling back to the Tier 1 manual form silently on fixture mode, a missing `VITE_STRIPE_PUBLISHABLE_KEY` (new, build-time, public, optional — deploy prerequisite: owner adds it to `book-renderer/.env.local` and to the production build's env), or any `loadStripe()` failure. Server contract (`ShippingAddressInput`, the `quote` op) unchanged. `npx tsc --noEmit` clean, `npx vitest run` 511/511 (new `order/__tests__/stripeAddressMapping.test.ts`), `npm run build:web` green (`check-web-bundle.mjs`'s fixture-string scan still zero hits; bundle grew ~530KB→~545KB minified from Stripe's loader). Interactive verification: fixture mode confirmed still manual-only with no console errors; the live Address Element path needs the owner's real publishable key to verify end to end. See "Implementation (address-entry autofill round)" above for the full breakdown. |
| 2026-09-09 | "Ships to" block on `OrderStatusScreen` (owner request): the quoted `shipping_address` shown under the price facts from `quoted` onward, with a "wrong address? reply right away" hint for `paid`/`rendering`/`submitted`/`in_production` — pairs with the KEPT 2h Prodigi edit window (owner decision, same day). `useOrderStatus` selects `shipping_address`; fixture rows carry it through. |
