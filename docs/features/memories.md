...
Journal entries in four formats: text with AI illustration, plain text only, user-uploaded photo/video with optional caption, or a kept audio clip with an editable description. Text saves to Postgres first for all types. Every analyzable memory gets an `analyze-emotion` pass, which as of 2026-08-24 runs the full `analyze-memory` enrichment pass (emotion + topics + labels + description + milestone claim in one call) rather than emotion alone — see [memory-analysis.md](./memory-analysis.md) for that pipeline's full behavior, data model, and extension guide; this doc keeps only the emotion-facing summary. `text_illustration`, `text_only`, and `audio` send text only (audio over its description + invisible transcript); `media` sends up to 4 images by position, preferring each asset's preview (photo previews AND video poster frames are both usable), so **video memories with a backfilled poster now get emotion too** — only a video with no poster at all still has no emotion. For `text_illustration` the emotion result also feeds the `generate-illustration` color palette. The memory type is derived from the creation form. See [audio-memories.md](./audio-memories.md) for the `audio` type's own capture flow, data model, and constraints — this doc only touches the sites where the type system itself is enumerated.
...
- **Editable AI mode:** the edit composer allows switching between `text_illustration` and `text_only`. Switching AI off hides the illustration immediately and saves the row as text-only without clearing `illustration_key`, `illustration_prompt`, or `illustration_status`. Switching it back on reveals a retained illustration without regeneration. If no retained illustration exists, saving with AI on sets the status to `pending` and starts generation. Pending/generating work is never duplicated, and switching it off does not cancel the background job.
  **Type-toggle trap:** "no retained illustration exists" is checked via `illustration_key`, but `shouldStartIllustration` also requires the retained `illustration_status` NOT already be `pending`/`generating` (`src/services/memories.ts` `updateMemory`, ~line 1136) — it assumes that status means a pipeline is already in flight. Illustration deferral (below) makes `pending` a parked state that can last minutes waiting on a portrait, so toggling AI off then back on during that window does **not** restart the pipeline; recovery relies entirely on the server retrigger or the client recovery loop described below, not this path. No code change is planned here — documented so a future agent doesn't "fix" it into a duplicate-trigger bug.
...
- Tapping a ready memory illustration opens the private image in the shared full-screen media viewer, where it can be pinched or double-tapped to zoom (see [media-memories.md](./media-memories.md)); closing returns to the detail card.
...
flowchart LR
  subgraph text_illustration
    ...
    DB1 --> Illustrate[generate-illustration dispatcher]
    Illustrate --> Emotion[Server emotion + palette when missing]
    Emotion --> Workflow[Cloudflare Workflow]
...
| `analyze-emotion` | After insert for `text_only` and photo/video-with-poster `media`; plus session backfill for analyzable memories missing emotion. Runs the full analysis pass (see [memory-analysis.md](./memory-analysis.md)); `analyze-memory` is the same handler under a forward-looking endpoint name for future clients. | JWT |
| `generate-illustration` | `text_illustration` dispatcher. It analyzes missing emotion/palette server-side, queues Workflow work, and is also portrait-retriggered/self-retriggered on deferral | JWT |
| `get-media-url` | Timeline/detail display for both illustration and media keys | JWT |

See TECH_SPEC §4.0, §4.2–4.3 for contracts.

### Durable illustration execution and recovery

`generate-illustration` is the sole client entry point. It authenticates the
caller, owns status/attempt transitions and prompt preparation in Supabase,
then creates a deterministic Cloudflare Workflow job. The Workflow receives
only a job ID, fetches private inputs during the generation step, calls OpenAI,
...
- New clients send `requestIntent: 'initial'`, `'recovery'`, or
  ...
  accepts both the older synchronous `{ success: true }` response and the
  queued `{ success: true, queued: true, jobId }` response while rollout is
  in progress.
- The two existing client recovery effects remain: loaded-list recovery is
  event-driven and detail recovery is driven by its 3-second status poll. They
  call the dispatcher and never update `illustration_status` in Postgres.
- Recovery clock is `illustration_generation_started_at ?? updated_at ??
  created_at`. A `pending` row is retried after 3 minutes, including an app
  death before dispatch; a `generating` row is retried only after 5 minutes
  30 seconds, preserving a 30-second Workflow-publication buffer.
- Installed legacy clients can still reset status around three minutes. The
  server publish CAS keys on the attempt ID, not status, so that reset cannot
...
- Model policy is `gpt-image-2.5-flare` primary then sequential `gpt-image-1.5`
  ...
  `gpt-image-1.5` and has its own latency smoke test before rollout.

Workflow state is retained for one day, but workflow event payloads and step
outputs contain IDs/metadata only—never memory text, prompt material, names,
reference keys, or image bytes. The canonical prompt persists only through the
authorized Supabase publication path.
...
- **Illustration deferral:** if no selected member has a *ready* portrait but at least one has a **fresh in-flight** one (`illustrated_profile_status` `pending`/`generating`, not deletion-claimed, with a claimed attempt younger than five minutes thirty seconds from `generation_started_at`, or an unclaimed `pending` row younger than three minutes from immutable `created_at` — see [TECH_SPEC §4.3](../TECH_SPEC.md#43-generate-illustration) for the exact predicate), `generate-illustration` returns `409 PORTRAITS_NOT_READY` instead of failing. This is the onboarding-guaranteed case: creating the first family member fires portrait generation in the background, and the first memory is usually captured before it finishes. The memory's status reset is **key-aware**: no retained `illustration_key` → resets to `pending` (attempt id cleared, parked like a fresh unillustrated memory); a retained `illustration_key` (e.g. regenerate after retagging) → stays `ready`, keeping the old image visible instead of an indefinite shimmer. `generate-portrait-illustration`'s background task retriggers up to 3 waiting memories per family (`illustration_status = 'pending'`, `created_at` at least 30s old to avoid racing a brand-new memory's own emotion analysis) on every exit path once a portrait resolves; the deferring request itself also self-retriggers once if its post-reset re-check finds the portrait already `ready`. See TECH_SPEC §4.1/§4.3 for the full mechanics.
  - **Client is silent about deferral, except on an explicit action.** `runMemoryIllustrationPipeline` (`src/services/memories.ts`) treats `PORTRAITS_NOT_READY` as success-shaped (`return null`, no `console.warn`) for the fire-and-forget create/update pipeline and the recovery loop below — a deferred memory looks the same as one still working. The detail screen's explicit regenerate action (`regenerateMemoryIllustration`) is the one exception: it surfaces the code distinctly, and the screen shows a non-error notice ("Character portraits are still generating — the illustration will finish automatically.") rather than a failure toast, so a deliberate tap doesn't just go quiet.
  - **Client backstop is the existing recovery loop, not a new mechanism.** `needsIllustrationRecovery` uses `illustration_generation_started_at ?? updated_at ?? created_at`: pending recovery begins at 3 minutes, and generating recovery begins at 5 minutes 30 seconds. Its existing list/detail effects call `retryMemoryIllustration`, which invokes the server with `requestIntent: 'recovery'`; it does not write status, timestamps, or attempt IDs. The dispatcher re-runs missing emotion analysis before claiming, so a failed initial analysis can recover. **Pacing differs by screen:** the timeline/list recovery effect is event-driven while the memory-detail effect is clock-driven via the 3s detail query poll. Both remain bounded to loaded/open memories and users with edit rights; this loop complements rather than replaces portrait completion retriggers.
