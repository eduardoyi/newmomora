...
| `generate-illustration` | After emotion analysis, `text_illustration` only; also server-retriggered by `generate-portrait-illustration` and self-retriggered on deferral (see Illustration deferral below) | JWT |
...
- **Illustration deferral:** if no selected member has a *ready* portrait but at least one has a **fresh in-flight** one (`illustrated_profile_status` `pending`/`generating`, not deletion-claimed, claimed-recency within the portrait function's 15-minute reclaim window or an unclaimed row within a ~5-minute `created_at` grace — see [TECH_SPEC §4.3](../TECH_SPEC.md#43-generate-illustration) for the exact predicate), `generate-illustration` returns `409 PORTRAITS_NOT_READY` instead of failing. This is the onboarding-guaranteed case: creating the first family member fires portrait generation in the background, and the first memory is usually captured before it finishes. The memory's status reset is **key-aware**: no retained `illustration_key` → resets to `pending` (attempt id cleared, parked like a fresh unillustrated memory); a retained `illustration_key` (e.g. regenerate after retagging) → stays `ready`, keeping the old image visible instead of an indefinite shimmer. `generate-portrait-illustration`'s background task retriggers up to 3 waiting memories per family (`illustration_status = 'pending'`, `created_at` at least 30s old to avoid racing a brand-new memory's own emotion analysis) on every exit path once a portrait resolves; the deferring request itself also self-retriggers once if its post-reset re-check finds the portrait already `ready`. See TECH_SPEC §4.1/§4.3 for the full mechanics.
  ...
  - **Client backstop is the existing recovery loop, not a new mechanism.** `needsIllustrationRecovery`/`isIllustrationPendingTooLong` (`src/utils/memories.ts`, `ILLUSTRATION_GENERATION_STALE_MS` = 3 min) already treat a stale `pending` row the same as a stale `generating` one, so once a deferred memory has sat at `pending` for 3+ minutes, `useMemories.ts`'s recovery effects retry it via `retryMemoryIllustration` → the same pipeline. A repeat `PORTRAITS_NOT_READY` resolves as success and just re-parks the memory with a fresh `updated_at`, so the next attempt is naturally ≥3 minutes later, not a tight loop — but note the redundant `analyze-emotion` re-run this path re-triggers (see the "Emergent type logic" pipeline note below; `analyze-emotion`'s cooldown is in-memory-per-isolate and gives no cross-cycle protection, so each recovery retry really does re-analyze). **Pacing differs by screen:** the timeline/list recovery effect (`useMemories.ts` ~line 515) is event-driven — it only re-runs when the `memories` array identity changes, not on a timer — while the memory-detail recovery effect (~line 699) is genuinely clock-driven via the detail query's 3s `refetchInterval`. Both stay bounded to loaded/open memories and to users with edit rights (A7 above); this loop is a backstop behind the server-side retrigger, not a replacement for it, and `useGenerationStatusPolling.ts` deliberately does not duplicate it.
...
- Reference images are capped to **1024px** max edge server-side before the OpenAI edit call. A one- or two-character memory uses the provider default output quality and a single `gpt-image-2` edit. A three-or-more-character memory requests `medium` quality explicitly and starts the compatible `gpt-image-1` reference-aware edit after 55 seconds if the primary has not settled; the first successful edit wins. This tail hedge is deliberately limited to the expensive multi-reference path. New output explicitly requests compressed WebP so its bytes match the immutable `.webp` R2 key and `image/webp` metadata.
- Each `generate-illustration` claim has a **120-second cancellable pre-finalization deadline** across lookup, safety rewriting, reference reads, and OpenAI edit work. Its timer accounts for earlier request processing and stays below Supabase Edge Functions' documented 150-second **request-idle limit**, leaving intended ~30-second headroom for an un-aborted immutable upload and checked DB publication; publication itself is not hard-capped by the deadline. A timeout returns `GENERATION_TIMEOUT` and reaches the claim-token-guarded cleanup instead of leaving a durable `generating` state. If a final CAS already succeeded, its `ready` state remains authoritative. A non-retryable provider 4xx/policy rejection does not trigger another model; transient failures, including 408, 409, 429, and 5xx, can try the alternate reference-aware edit model, but this flow never falls through to a text-only generation once portraits are supplied because that would break character continuity. Phase telemetry is limited to memory id, phase, and duration; it never includes memory content, prompts, member data, URLs, storage keys, or credentials.
