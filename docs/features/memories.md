...
- Each `generate-illustration` claim has a **120-second cancellable pre-finalization deadline** across lookup, safety rewriting, reference reads, and OpenAI model fallbacks. Its timer accounts for earlier request processing and stays below Supabase Edge Functions' documented 150-second **request-idle limit**, leaving intended ~30-second headroom for an un-aborted immutable upload and checked DB publication; publication itself is not hard-capped by the deadline. A timeout returns `GENERATION_TIMEOUT` and reaches the claim-token-guarded cleanup instead of leaving a durable `generating` state. If a final CAS already succeeded, its `ready` state remains authoritative. Phase telemetry is limited to memory id, phase, and duration; it never includes memory content, prompts, member data, URLs, storage keys, or credentials.
...
| Deno | `supabase/functions/analyze-emotion/index.test.ts`, `generate-illustration/index.test.ts` (claim-timeout cleanup), `notify-memory-engagement/index.test.ts`, `_shared/member-mentions.test.ts`, `_shared/illustration-references.test.ts` (abort propagation), `_shared/openai.test.ts` (fallback behavior and abort stop), `_shared/image-bytes.test.ts` |
