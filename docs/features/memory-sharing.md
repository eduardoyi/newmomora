...
| `supabase/functions/compose-share-card/layout.test.ts` | SVG-tree snapshot per memory-type variant (quote/spread), date-label formatting, quote-glyph bottom-margin pinned to the exact `+8` logical-unit value (user-feedback follow-up), flag/mixed-emoji captions still produce real glyph path data (pre-`graphemeImages` shaping floor, not a regression guard for the image substitution itself — see render.test.ts for that); **`objectFit: cover` (tail fix, pass 3):** both the hero AND portrait `<img>` nodes declare it in their tree style, and — against a real satori render with a genuinely tall (aspect_ratio ~0.56) fixture clamped into a 3:4 block — the embedded `<image>`'s own geometry shows a uniform-scale, center-cropped embed (width matches the box, height overflows with a negative y offset), not a plain stretch |
...
| `supabase/scripts/backfill-share-cards.test.ts` | Pure helpers only: **`needsWarming` (tail fix, pass 3 — the backfill-gap fix):** null key needs warming, a key at the CURRENT `DESIGN_VERSION` (imported from compose-share-card/index.ts, not duplicated) does not, an OLDER-version key does; `shapeMemoryTargets`/`shapeMediaAssetTargets` (row → target shaping now filtering on `needsWarming`, not just null; dangling-FK-join defensive drop, independent of the staleness filter), `describeTarget` (id-only, no content), `decideWarmRetry` (retries ONLY 546 while under the attempt cap, gives up immediately on 429/4xx/5xx/network-error/attempt-cap-reached, respects a custom `maxAttempts`) |
...
| 2026-08-06 | Tail fix, pass 3 (device-verified bug): a tall photo rendered visibly SQUISHED on the share card (satori's `<img>` defaults to non-uniform stretch to exactly fill its given width/height) while the in-app card showed the SAME photo correctly CROPPED (RN `resizeMode: 'cover'`). `clampMediaAspectRatio` was never the bug and stays unchanged — it bounds the image BLOCK's aspect ratio as part of the pixel-budget mitigation; the bug was the `<img>` not reproducing the app's crop semantics for a block whose (clamped) aspect ratio differs from the source's own true aspect. Fix: `objectFit: 'cover'` on both the hero (`imageBlockNode`) AND portrait (`memberAvatarNode`) `<img>` nodes, layout.ts — satori supports it, verified against a real tall (9:16-ish, aspect_ratio ~0.56) photo fixture clamped into a 3:4 block: the embedded `<image>`'s own geometry showed a uniformly-scaled, center-cropped embed (height overflowing the box with a negative y offset), not a plain 0/0/width/height stretch. **`DESIGN_VERSION` 3 → 4** (a REAL visible-content change — every clamped-aspect image block on every already-stored card up to version 3 is squished and must regenerate). **Backfill gap closed in the same pass:** `backfill-share-cards.ts`'s candidate selection was `share_card_key IS NULL` only, so a `DESIGN_VERSION` bump never proactively re-warmed anything (a version-stale key is non-null) — it only got picked up lazily, one real share at a time. Selection now also includes version-stale rows, via the SAME `isFreshShareCardKey` compose-share-card/index.ts already uses for its own cache-hit check (imported directly, not re-implemented, so the script's notion of "stale" can never drift from the function's own). |
| 2026-08-06 | Source-of-the-mismatch fix (both prior rows above worked around the symptom on the READ side; this fixes the WRITE side so it stops accumulating): `generate-illustration`/`generate-portrait-illustration` now sniff the REAL bytes OpenAI's images/edits endpoint returns (`_shared/image-bytes.ts`'s new `sniffImageFormat`/`resolveRealImageBytesForStorage`, same magic-byte signatures as this file's `resolveImageMimeType`) instead of trusting the requested `output_format: 'webp'`. A PNG mismatch is re-encoded to JPEG (no vendored webp ENCODER exists — `@jsquash/webp` here is decode-only) at generation time, collapsing new illustrations/portraits from ~2MB to ~150-350KB immediately, not just once a share card happens to compose them. `generate-illustration`'s key now carries the REAL extension (`buildMemoryIllustrationKey`'s new optional extension param; `storage-keys.ts`'s illustration patterns now accept `.jpg` alongside `.webp`) — zero DB/RPC changes needed there since its final key was never pre-committed before generation. `generate-portrait-illustration` keeps its `.webp`-suffixed key (the legacy `claim_family_member_portrait_generation` RPC validates that exact extension server-side BEFORE generation runs, so the key can't vary post-generation without a dedicated migration — tracked separately, not done here) but still gets the SIZE fix and correct R2 Content-Type metadata; downstream consumers (this file's `resolveImageMimeType`) already sniff bytes over trusting the extension, so this remains safe. `supabase/scripts/backfill-portrait-reencode.ts` re-encodes existing mismatched rows (sniff-based, idempotent, never-reuse-key for the two versioned columns; see docs/features/memories.md and docs/features/portrait-timeline.md). |
| 2026-08-06 | Same source-of-the-mismatch fix, Cloudflare worker variant: `cloudflare/memory-illustration-worker/src/image-output.ts` sniffs OpenAI images/edits output the same way (`sniffImageFormat`, kept in sync with `_shared/image-bytes.ts`) but its `outputKey` is minted Supabase-side and always `.webp`, woven through the bridge upload-lease protocol, so it can't rename the key like the Supabase fix above — instead `resolveOutputBytesForR2` RE-ENCODES non-webp bytes to real webp via the `env.IMAGES` binding (already used for reference loading) before every `MEMORY_ILLUSTRATIONS`/`CHARACTER_PORTRAITS` PUT, failing open to the sniffed real content type if re-encoding itself throws. Wired into both `workflow.ts` and `portrait-workflow.ts`'s `runImageAttempt`; see docs/cloudflare-illustration-workflows.md. |
