...
- **`env` super property.** `createClient()` registers `{ env: __DEV__ ? 'dev' : 'prod' }` once at construction. Every dashboard should filter `env = prod` — launch funnels are small-N and a few developer/simulator runs would otherwise skew them.
