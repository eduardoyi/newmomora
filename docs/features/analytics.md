...
- **`env` super property.** `registerEnvSuperProperty()` registers `{ env }` at client construction **and again after every `resetAnalytics()`** — `posthog.reset()` wipes registered super properties along with the distinct id (verified against a live project: post-sign-out events arrived without `env`), and without re-registration every post-sign-out user would silently drop out of the env-filtered dashboards. The value comes from `resolveAnalyticsEnv()`: `EXPO_PUBLIC_APP_ENV === 'production'` → `prod`, any other value → `dev`, `__DEV__` fallback when the var is unset. **`__DEV__` alone is not a safe signal** — a dev-client launched without Metro runs its embedded release bundle, so standalone device test runs self-report as prod (observed live). `EXPO_PUBLIC_APP_ENV` is set per EAS environment (`production` only on the production environment; `development`/`preview` elsewhere). Every dashboard filters `env = prod` — launch funnels are small-N and a few developer/simulator runs would otherwise skew them.
...
| `memory_search_result_opened` | `matched_in` (`text`/`voice`/`details`/`chip`), `position`, `has_text`, `person_count`, `has_feeling` | Search result row press (never the query text) |
