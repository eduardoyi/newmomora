-- Harden replace_memory_media_assets against fractional durationMs values.
--
-- iOS expo-image-picker reports video `duration` as a fractional Double in
-- milliseconds (asset.duration.value / timescale * 1000 -- see
-- node_modules/expo-image-picker/ios/VideoUtils.swift:24). The client now
-- rounds at the source (src/components/memory-media-picker.tsx,
-- src/services/memories.ts mediaAssetsToRpcPayload), but the RPC must not
-- depend on every current and future caller doing so: `nullif(...)::integer`
-- throws on fractional text like '21894.667', which failed every video post
-- whose duration wasn't an exact integer millisecond count. Recreated
-- (identical to the 20260715140000 version) with one line changed --
-- `round(...)::numeric)::integer` accepts fractional input from any older
-- client forever, matching the defensive rounding already applied
-- client-side.
...
begin
  ...
  -- without sending the newly added aspectRatio/previewObjectKey fields.
  select
  ...
  loop
    ...
    -- Changed line: round through numeric before casting to integer so a
    -- fractional ms value (e.g. '21894.667') doesn't throw -- see migration
    -- header comment.
    ...
    asset_preview_object_key := coalesce(
      nullif(asset->>'previewObjectKey', ''),
      nullif(existing_preview_object_keys->>asset_key, 'null')
    );
    ...
    -- Same ownership/pattern check as the primary object key: a preview
    -- lives at the same asset path, only the filename differs. Garbage or
    -- foreign preview keys must not be persistable any more than garbage or
    -- foreign object keys are.
    if asset_preview_object_key is not null
      and not (
        asset_preview_object_key ~ ('^' || caller_prefix || '/media/[A-Za-z0-9_-]{1,128}[.](jpg|jpeg|png|heic|heif|webp|mp4|mov)$')
      )
    then
      raise exception 'Invalid preview object key' using errcode = '22023';
    end if;
