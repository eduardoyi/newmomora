-- Restore media metadata persistence after
-- 20260729130000_onboarding_anonymous_lockdown.sql recreated this RPC from
-- an older definition. Without these fields, newly uploaded photos lose
-- their natural aspect ratio and timeline cards fall back to 4:3.
--
-- Keep the anonymous-account guard from the lockdown migration while
-- restoring the aspect-ratio and preview metadata introduced by the media
...
begin
  ...
  -- without sending the newer aspectRatio/previewObjectKey fields.
  select
