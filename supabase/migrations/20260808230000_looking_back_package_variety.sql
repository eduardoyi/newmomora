-- Keep each daily Looking Back set varied without forcing specific recipe
-- combinations. A recipe type may contribute at most one package per set;
-- member-at-age candidates prefer a subject who has not appeared in that
-- recipe during the preceding three days.
