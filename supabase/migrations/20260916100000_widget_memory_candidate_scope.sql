-- Home-screen widget: tighten the deployed candidate RPC to image-backed rows.
--
-- 20260915140000_widget_memory_candidates.sql is already deployed. Keep this
-- follow-up as a replacement migration so the existing auth, RLS, timezone,
-- safety, and bounded sampling contract remains stable while the widget only
-- receives memories it can render as an image card.
