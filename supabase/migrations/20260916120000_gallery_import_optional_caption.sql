-- Let a parent clear the AI-drafted caption on a gallery-import suggestion.
--
-- The candidate table required a non-empty caption because every AI draft
-- ships with one, but the review UI lets a parent delete that draft entirely
-- before saving. Today that save fails with a check violation. A cleared
-- caption should behave exactly like the composer's own media-only memories:
-- the candidate keeps a not-null, empty-string caption (candidates are never
-- themselves nullable-content rows), and finalize_gallery_import_candidate
-- turns that empty string into memories.content = null, matching
-- public.memories' own "content is null or trim(content) <> ''" rule from
-- 20260525143000_memory_types.sql. AI-generated drafts are unaffected: the
-- workflow-gallery-import-bridge publication path (validCandidates in
-- supabase/functions/workflow-gallery-import-bridge/index.ts) still requires
-- a non-empty caption before a candidate is ever created, so this only
-- relaxes the user-edit path.

alter table public.gallery_import_candidates
  drop constraint if exists gallery_import_candidates_caption_check,
  add constraint gallery_import_candidates_caption_check
    check (char_length(btrim(caption)) <= 1000 and caption !~ '[[:cntrl:]]');

-- Recreate finalize_gallery_import_candidate (authoritative body as of
-- 20260823100000_gallery_import_continuous.sql) unchanged except for the
-- memories.content value: an empty/whitespace-only candidate caption now
-- inserts as null instead of as an empty string.
...
begin
  ...
  insert into public.memories (id,user_id,family_id,content,memory_date,memory_type,media_key,media_content_type,illustration_status,emotion,creation_source)
    values (v_lease.memory_id,v_candidate.actor_id,v_candidate.family_id,nullif(btrim(v_candidate.caption),''),v_candidate.memory_date,'media',v_first_key,v_first_type,'none',v_candidate.emotion,'gallery_import')
