-- Preview image variants (Workstream C, performance-optimizations plan).
-- Adds a derived, bandwidth-friendly JPEG preview key alongside each media
-- asset's untouched original. Previews are generated client-side (longest
-- edge <= 1280px) and referenced from memory_media; originals are NEVER
-- downscaled (keepsake product -- full quality preserved for future
-- export/print). List-view surfaces prefer the preview and fall back to the
-- original when it is absent (legacy rows, videos, the no-upscale guard, or
-- a failed preview upload -- fail-open). See docs/features/media-memories.md.

alter table public.memory_media
  add column preview_object_key text;

-- Recreated (not just altered) so the new preview_object_key argument gets
-- the identical ownership/pattern validation as object_key -- otherwise a
-- caller could persist a garbage or foreign preview key.
