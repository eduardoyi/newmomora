-- Family activity rows for memories with no image (text-only, audio, or
-- an illustration that never landed) render the same type-aware tiles the
-- calendar stamp and member-profile thumb use -- an emotion-tinted quote
-- mark or a SoundTile -- instead of a blank grey box. Adds `memory_type`
-- and `memory_emotion` to get_family_activity so the client can pick and
-- tint that tile. Return table changes shape again, so drop + recreate;
