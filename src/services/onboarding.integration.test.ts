...
describe('commitOnboarding', () => {
  beforeEach(() => {
    ...
    mockedFetchFamilyMembers.mockResolvedValue({
      data: [{ id: 'member-lila' }, { id: 'member-miguel' }] as never,
      error: null,
    });
  ...
  it('atomically commits the family, children, and tagged first memory through the RPC', async () => {
    ...
    expect(mockedSupabase.rpc).toHaveBeenCalledWith('commit_onboarding', expect.objectContaining({
      p_family_name: "Lila & Miguel's Family",
      p_kid_names: ['Lila', 'Miguel'],
      p_tagged_kid_indexes: [0],
      p_capture: expect.objectContaining({
        text: 'She lined up her stuffed animals to watch for the garbage truck.',
        hasMedia: false,
      }),
    }));
    // Family + kids + memory committed before notification prefs apply.
    expect(mockedPatchOnboardingDraft).toHaveBeenCalledWith(expect.objectContaining({ committedFamilyId: 'family-1' }));
  ...
  it('falls back to tagging the first kid when no chip is selected', async () => {
    ...
    expect(mockedSupabase.rpc).toHaveBeenCalledWith('commit_onboarding', expect.objectContaining({
      p_tagged_kid_indexes: [],
    }));
  });

  it('builds a pendingMediaUpload payload for the atomically-created media placeholder', async () => {
    ...
    expect(mockedSupabase.rpc).toHaveBeenCalledWith('commit_onboarding', expect.objectContaining({
      p_capture: expect.objectContaining({ hasMedia: true, contentType: 'image/jpeg' }),
    }));
    expect(result.error).toBeNull();
    ...
    expect(mockedPatchOnboardingDraft).toHaveBeenCalledWith(expect.objectContaining({
      captureCommitted: false,
      pendingMediaMemoryId: result.data?.memoryId,
    }));
  });
  ...
  it('fails closed when the atomic onboarding transaction rejects, without client-side partial writes', async () => {
    mockedSupabase.rpc.mockResolvedValue({ data: null, error: { message: 'Could not create Miguel' } } as never);

    const result = await commitOnboarding(buildDraft());

    expect(result.data).toBeNull();
    expect(result.error?.message).toBe('Could not create Miguel');
    expect(mockedSupabase.rpc).toHaveBeenCalledWith('commit_onboarding', expect.anything());
    expect(mockedCreateMemory).not.toHaveBeenCalled();
    // The idempotency key is persisted before the RPC so a lost response can
    // be retried safely; the family commit marker is never persisted.
    expect(mockedPatchOnboardingDraft).not.toHaveBeenCalledWith({ committedFamilyId: expect.any(String) });
  });
  ...
  describe('returning owner who already has a family (docs/features/onboarding.md decision 18)', () => {
    it('saves the captured memory into the existing family instead of creating a second one', async () => {
      ...
      mockedSupabase.rpc.mockResolvedValueOnce({ data: { has_write_access: true }, error: null } as never);
      ...
      expect(mockedPatchOnboardingDraft).toHaveBeenCalledWith(expect.objectContaining({ committedFamilyId: 'family-existing' }));
    });
    ...
    it('routes a lapsed owner with a deferred media capture to resubscribe before queueing the upload', async () => {
      mockedFetchMyFamilyMemberships.mockResolvedValue({
        data: [
          {
            id: 'membership-1',
      ...
      mockedSupabase.rpc.mockResolvedValue({ data: { has_write_access: false }, error: null } as never);

      const result = await commitOnboarding(buildDraft({
        capture: {
          text: 'A photo from before the subscription check.',
          mediaUri: 'file:///photo.jpg',
          mediaContentType: 'image/jpeg',
          taggedKidIndexes: [0],
        },
      ...
      expect(mockedPatchOnboardingDraft).not.toHaveBeenCalledWith({
    ...
    it('never resolves a soft-deleted family as "existing"', async () => {
      ...
      mockedSupabase.rpc.mockResolvedValue({
        data: [{ family_id: 'family-new', memory_id: 'memory-1', is_new_family: true }],
        error: null,
      } as never);
