/** Identical to cloudflare/memory-book-worker/src/crypto.ts -- duplicated
 * rather than imported (Cloudflare Workers each build their own bundle;
 * this repo's convention, per that file's own precedent, is a small
 * self-contained copy per Worker rather than a cross-package import). */
