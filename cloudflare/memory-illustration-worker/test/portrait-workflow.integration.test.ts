...
import type { PortraitWorkflowJobInput, PortraitWorkflowJobInputV2 } from '../src/types';
...
function job(overrides: Partial<PortraitWorkflowJobInputV2> = {}): PortraitWorkflowJobInputV2 {
  return {
    jobId: JOB_ID,
    usageRequestId: '50abcc52-5c0d-4b7b-86d4-1b3a0a661223',
...
describe('portrait Workflow', () => {
  ...
  it('fails safely before OpenAI when the portrait bridge returns an old provider protocol', async () => {
  ...
  it('continues portrait publication when best-effort usage recording fails', async () => {
    const setup = createEnvironment();
    const { fetchMock } = bridgeAndOpenAiFetch(setup.workflowJob, {
  ...
  it('publishes a portrait when record_usage never resolves', async () => {
    const setup = createEnvironment();
    const { fetchMock } = bridgeAndOpenAiFetch(setup.workflowJob, {
    ...
    expect(setup.portraits.put).toHaveBeenCalledTimes(1);
  });
  it('uses style then source references, sends canonical WebP options, publishes, then deletes the previous portrait', async () => {
    ...
    expect(operations.map((operation) => operation.operation)).toEqual([
      'get_input', 'reserve_attempt', 'record_usage', 'authorize_upload', 'record_upload_complete', 'publish', 'retrigger_memories',
    ]);
    expect(operations.find((operation) => operation.operation === 'record_usage')).toMatchObject({
